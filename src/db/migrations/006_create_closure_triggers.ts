...
export async function up(knex: Knex): Promise<void> {
  ...
  // update group_group_closure to reflect transitive relationships
  await knex.raw(`
    ...
    BEGIN
      ...
      INSERT INTO group_group_closure (ancestor_group_id, descendant_group_id, depth)
      ...
      FROM group_group_closure a
      CROSS JOIN group_group_closure d
  ...
  await knex.raw(`
    ...
    BEGIN
      IF NEW.user_id IS NOT NULL THEN
        ...
        INSERT INTO group_user_closure (group_id, user_id)
      ...
      IF NEW.child_group_id IS NOT NULL THEN
        ...
        FROM group_user_closure gmc
        CROSS JOIN group_group_closure gap
  ...
  await knex.raw(`
    ...
    BEGIN
      IF OLD.user_id IS NOT NULL THEN
        ...
        DELETE FROM group_user_closure WHERE user_id = OLD.user_id;
        ...
        LEFT JOIN group_group_closure gap ON gap.descendant_group_id = gm.group_id
        WHERE gm.user_id = OLD.user_id
        ON CONFLICT DO NOTHING;
      END IF;

      IF OLD.child_group_id IS NOT NULL THEN
        -- Full rebuild of group_user_closure for affected groups
        -- This is the nuclear option but is correct; optimize later if needed
        DELETE FROM group_user_closure
        WHERE group_id IN (
          SELECT ancestor_group_id FROM group_group_closure
        ...
        JOIN group_group_closure gap2 ON gap2.ancestor_group_id = g.descendant_group_id
