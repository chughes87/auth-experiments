...
The most important database-level invariant. If adding a group nesting edge would create a cycle (A contains B, B contains A), the insert is rejected. Uses `group_ancestor_closure` (the group→group closure table) for cycle detection.
...
Triggers that maintain `page_tree_closure` on page insert/delete/move, `group_ancestor_closure` on group nesting changes, and `group_membership_closure` (flattened user→group) derived from `group_members` + `group_ancestor_closure`. These ensure the precomputed closure tables never drift from the source of truth.
...
Files: `specs/PermissionResolution.tla`, `specs/PermissionResolution.cfg`
